...
class passportScan:
    ...
    def __init__(self, data):
        ...
        if (len(data) > 88):
            data = data.replace(' ', '')
            firstline = data[-88:-44]
        else:
            firstline = data[:44]
        secoundline = data[-44:]
        if 'O' in secoundline:
            secoundline = secoundline.replace('O', '0')
        # to check type of passport
        if (firstline[0] != "P"):
            raise ValueError("There is another type")

        stype = '0' if firstline[1] == 'O' else firstline[1]
        self.subtype = "None" if stype == '<' else stype
        ...
        self.passportNum = secoundline[:9].strip('<')
        ...
        self.personalNum = secoundline[28:42].strip('<')
    ...
    def detail(self):
        print("Type = %s" % self.TD)
        print("Sub-type = %s" % self.subtype)
        print("Name = %s" % self.name)
        print("Surname = %s" % self.surname)
        print("Passport number = %s"%self.passportNum)
        print("Nationality = %s"%self.nationality)
        yy = self.dateOfBirth[:2]
        mm = self.dateOfBirth[2:4]
        dd = self.dateOfBirth[4:]
        print("Date of birth = {0}-{1}-{2}".format(yy,mm,dd))
        print("Sex = %s"%self.sex)
        yy = self.expiry[:2]
        mm = self.expiry[2:4]
        dd = self.expiry[4:]
        print("Expiration date = {0}-{1}-{2}".format(yy, mm, dd))
        print("Personal number = %s"%self.personalNum)
